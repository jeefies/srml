...
class MailBox:
    ...
    def __init__(self, host=None, mails=None, ssl=True):
        ...
        Notice, if you want to change one of the protocol's host, you can set like this:
            box = MailBox('163.com') # also box = (mails = ('myaccount@163com', 'MYPASSWORD'))
            # if host is not given but mails does, MailBox would autoanalyze the host of the mail
        ...
        Here is an example usage:
            ...
            msg = box.createMessage('name') # Do not create more then one message at once.
            # the only argument is for the name to send 
            msg.subject = "Subject"
            msg.body = 'This is the body part, if the html or mkd is given, this would be shown'
            msg.html = "<h1>HTML CONTENT TITLE</h1>" # Advice that it's better not to use html to modify the content of the message
            msg.mkd = "*markdown italic*, and __BOLD__"
            msg.reci = ['receive_email@some.com', 'another_receive@anaother.com']
    ...
    def login(self, mail=None, passwd=None):
        ...
        "if mail and passwd is not given, it would use the info given when creating the mailbox"
        if mail and passwd:
